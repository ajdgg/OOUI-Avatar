...
const init = (Toolset) => {
    ...
    const InitCanvas = async (base64) => {
        try {
            ...
            Toolset.setMsgBoxText(e.message);
    ...
    const displayAvatar = () => {
        ...
        picker.onchange = (event) => {
            ...
            if (!FileType.includes(file.type)) {
                Toolset.setMsgBoxType('error');
                Toolset.setMsgBoxText(mw.msg('avatar-invalid'));
                Toolset.setMsgBoxShow(true);
                return;
            }
            ...
            if (file) {
                ...
                reader.onloadend = () => {
                    ...
                    Toolset.setMsgBoxType('notice');
                    Toolset.setMsgBoxText(mw.msg('uploadavatar-hint'));
                    Toolset.setMsgBoxShow(true);
                    cropperWidth = cropper.offsetWidth;
    ...
    determineUploadBtn.onclick = () => {
        ...
        clippingArea.style.display = "none";
        currentAvatar.style.display = 'block';
        const url = window.location.search === '' ? window.location.pathname : window.location.pathname + window.location.search;
        fetch(url, {
            method: 'POST',
            headers: {
